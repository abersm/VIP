...
if (FALSE) {
  as_dropdown <- function(btn, ..., class = "dropdown-menu p-3", placement = "right", show_arrow = FALSE) {
    btn_group_class <- "btn-group"
    if (!is.null(placement) && placement != "") {
      placement <- switch(
        placement,
        right = "dropend",
        left = "dropstart",
      ...
      btn_group_class <- paste(btn_group_class, placement)
    }
    btn$attribs$type <- "button"
    btn$attribs$class <- paste(btn$attribs$class, "dropdown-toggle")
    btn$attribs$`data-bs-toggle` <- "dropdown"
    btn$attribs$`aria-expanded` <- "false"
    btn$attribs$`data-bs-auto-close` <- "outside"
    if (!show_arrow) {
      btn <- shiny::tagList(shiny::tags$head(shiny::tags$style(shiny::HTML(sprintf("#%s::after{display:none;}", btn$attribs$id)))), btn)
    }
    dropdown_content <- shiny::tags$form(class = class, ...)
    shiny::tags$div(
...
popover_btn <- function(
  ...
  #btn <- add_tooltip(shiny::icon("virus", style = sprintf("height:26px;width:26px;border-radius:1rem;padding:0.3rem;background:%s;color:white;width:min-content;", header_border_color)), hover_text)
  btn <- add_tooltip(btn, hover_text)
  bslib::popover(
    title = title,
    trigger = btn,
    placement = "right",
