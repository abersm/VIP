...
#' @param raw_data,meta Data frames for raw data and final results of meta-analysis respectively
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$export_data <- shiny::downloadHandler(
      ...
      content = function(file) {
        out <- .prepare_meta_analysis_data(raw_data, meta, virus = input$virus, population = input$population, outcome = input$outcome, input$study_design, low_rob = input$rob)
        out <- dplyr::arrange(out, .data$virus, .data$outcome, .data$population, .data$study_design, .data$is_meta, .data$estimate)
